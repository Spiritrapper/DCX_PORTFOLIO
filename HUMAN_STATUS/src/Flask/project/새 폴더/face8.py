    ...
    while True:
        ...
        for i, region in enumerate(regions):
            ...
            # Tester3 테이블에 2초마다 상태 업데이트
                      # Tester3 테이블에 데이터 삽입 또는 업데이트
            try:
                current_time = time.time()
                if current_time - user_state["last_update_time"] >= 2:
                    status = 1 if user_state["ALERT"] else 0
                    # INSERT INTO ... ON DUPLICATE KEY UPDATE 구문으로 변경
                    query = (f"INSERT INTO Tester3 (region_id, status{i + 1}, timestamp) "
                             f"VALUES (%s, %s, NOW()) "
                             f"ON DUPLICATE KEY UPDATE status{i + 1} = %s, timestamp = NOW()")
                    val = (i + 1, status, status)
                    cursor.execute(query, val)
                    db.commit()
