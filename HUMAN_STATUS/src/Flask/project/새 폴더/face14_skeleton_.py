...
def detect_sleepiness(ear, angle):
    shoulder_angle = 0  # 초기값 설정
    if ear < 0.25 or angle < 167:
        return True
    return False


# detect_pose 함수 내부 수정
...
def capture_video():
    ...
    shoulder_angle = 0  # Initialize shoulder_angle here
    ...
    while True:
        ...
        for i, region in enumerate(regions):
            ...
            for rect in rects:
                ...
                # 여기에서 detect_pose 함수를 호출합니다.
                # 수정된 코드
                updated_frame, _ = detect_pose(frame, pose, body_language_model)
                frame = updated_frame

               # print(type(frame))  # NumPy 배열이어야 합니다.
            ...
            # 한글 텍스트를 그리기 전에 frame이 올바른 형식인지 확인합니다.
